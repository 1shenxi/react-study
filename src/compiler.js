...
((React) => {
  ...
  setInterval(() => {
    ...
    ReactDOM.render(<Clock date={new Date()}></Clock>, document.querySelector('#root'))
  }, 1000)

  // 对比类组件。props 存在与类实例上.（也就是 this 上）
  class ClockClassComp extends React.Component {
    render() {
      return (
        <div className="clock">
          <p>ClockClassComp 当前时间</p>
          {/* jsx 中对于数据的使用采用 `{}` 来进行动态插入 */}
  ...
  setInterval(() => {
    // 每次动态的去修改 date 中的值。会发现只有 date 那部分的数据一直被修改。这就是虚拟DOM 的只修改局部
    ReactDOM.render(<ClockClassComp date={new Date()}></ClockClassComp>, document.querySelector('#root1'))
  }, 1000)

  // 关键2: Props 只具有可读性。在组件中修改 props，不会被修改成功。
  ...
  ReactDOM.render(<Clock1 date={new Date()}></Clock1>, document.querySelector('#root2'))

  // 关键3: state 是当前组件内部的一个数据集合，和 Props极其相似，只不过 state 是私有的。
  // 根据上面案例，会发现，计时器的会被重写多次。理想情况下，我们希望只写一次，因此: 
  class Clock2ClassComp extends React.Component {
    constructor(props) {
      super(props)
      // ⚠️注意：constructor 中初始化 state 可以直接给属性赋予值，之后只能通过 setState 赋值。
      this.state = {
        date: new Date()
      };
  ...
  ReactDOM.render(Wrap, document.querySelector('#root-compiler'))

  // 至于为什么不用 函数式组件 写，是因为，目前学到的函数式组件是不具有上下文环境的，因此，也就没有this，也就没有state，
  ...
  const Wrap1 = (
    <div style={{ background: 'skyblue' }}>
      <Clock2ClassComp msg="123" />
    </div>
  )
  ReactDOM.render(Wrap1, document.querySelector('#root1-compiler'))
  // 2. 2s 后清空 document.querySelector('#root1-compiler') 中的虚拟DOM
  setTimeout(() => {
    ReactDOM.render(null, document.querySelector('#root1-compiler'))
  }, 2000)

  // 这时候，会发现，html 中，已经没有Wrap1相关的真实DOM了，但是控制台下还是会一直打印 msg。
